...
export default function LoanSummary() {
  ...
  const summaryItems = [
    { label: 'Purchase Price (SPA)', value: formatCurrency(spaPrice), icon: '🏠' },
    { label: 'Down Payment', value: formatCurrency(downPaymentAmount), icon: '💸' },
    ...
    { label: 'Last Payment Due', value: lastPaymentDate.toLocaleDateString(), icon: '📆' },
    { label: 'Monthly Installment', value: formatCurrency(monthlyInstallment), icon: '📈' },
    { label: 'Total Interest Payable', value: formatCurrency(totalInterest), icon: '💰' },
  ...
  return (
    <>
      ...
      >
        ...
        {expandSummary ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
      </div>
