  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-lg shadow p-4 flex flex-col items-center justify-center">
      <div className="relative w-40 h-40">
        ...
        {tooltip.visible && (
          <div
            className="absolute text-xs px-2 py-1 bg-black text-white rounded shadow"
            style={{
              top: tooltip.y,
              left: tooltip.x,
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
          ...
          <div className="text-lg font-bold text-primary">{formatCurrency(firstMonth.total)}</div>
        </div>
      </div>

      <div className="mt-2 text-xs text-center space-y-1">
        <div className="flex items-center justify-center gap-1">
          <div className="w-3 h-3 rounded-sm bg-[#00a6ed]" /> Principal: {principalPct.toFixed(1)}%
        </div>
        <div className="flex items-center justify-center gap-1">
          <div className="w-3 h-3 rounded-sm bg-[#ca0b4a]" /> Interest: {interestPct.toFixed(1)}%
        </div>
      </div>
    </div>
  )
